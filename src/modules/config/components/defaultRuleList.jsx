...
class DefaultRuleList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      shopId:'',
    ...
    return [{
      ...
      render:(text, record) => {
        ...
        if(record.ruleId == 'bill-exception-1' && record.param){
          ...
          textHtml = '当商户在营业范围内净销售额同比下降'+record.param.downRate+'%或者上升'+record.param.upRate +'%，触发报警';
  ...
  _editRuleParam = (data) => {
    this.props.updateDefaultRuleParamsEdit(data);
    this.props.setDefaultRuleParams(true);
  }
  ...
  _hideModalRuleParamCancel = () => {
    ...
    this.props.setConfigDefaultVisibleFalse();
    this.props.setBatchConfigRuleIds([]);
    this.props.setShopEntityId([]);
    this.props.setBatchButtonDisabled();
  }

  _hideModalRuleParamOk = () => {

    if(this.props.shopEntityId == '' || this.props.ruleIds == 0){
      window.alert('请至少选择其中某一个报警类型，否则该报警策略不能生效!');
      return false;
    }
    this.props.setBatchConfigList();
    this.props.setConfigDefaultVisibleFalse();
    this.props.setBatchConfigRuleIds([]);
    //this.props.setShopEntityId([]);
    this.props.setBatchButtonDisabled();
  ...
  _onRowSelectChange = (selectedRowKeys, selectedRows) => {
    ...
    this.props.setBatchConfigRuleIds(selectedRowKeys);
  }
  ...
  componentWillReceiveProps(nextProps){
    if(this.props.editor && this.props.editor != nextProps.editor){
      this.setState({
        ...
        defaultRuleParams: nextProps.defaultRuleParams,
        shopEntityId: nextProps.shopEntityId,
        ruleIds: nextProps.ruleIds
      });
    }
  }
  ...
  render(){
    const rowSelection = {
      type: 'checkbox',
      selectedRowKeys: this.props.ruleIds,
    ...
    return (
      <Modal
        ...
        maskClosable={false}
      >
        <Table rowSelection={rowSelection} size="small" columns={this._columns()} dataSource={this.props.defaultRuleParams ||  []} pagination={false} loading={this.props.loading}/>
      </Modal>
    );
  }
};

DefaultRuleList.defaultProps = {
...
DefaultRuleList.propTypes = {
  defaultVisible: PropTypes.bool,
  defaultRuleParams: PropTypes.array,
  selectedRowKeys: PropTypes.array,
  shopEntityId: PropTypes.array,
